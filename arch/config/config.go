...
type ConnectionConfig struct {
	...
	IdleMaxLifetime     time.Duration `mapstructure:"idle_max_lifetime"`
	IdleMaxIdleConns    int           `mapstructure:"idle_max_idle_conns"`
	IdleMaxOpenConns    int           `mapstructure:"idle_max_open_conns"`
	ConnectRetries      int           `mapstructure:"connect_retries"`
	ConnectRetryDelay   time.Duration `mapstructure:"connect_retry_delay"`
	ConnectMaxIdleTime  time.Duration `mapstructure:"connect_max_idle_time"`
	ConnectMaxLifetime  time.Duration `mapstructure:"connect_max_lifetime"`
	ConnectMaxIdleConns int           `mapstructure:"connect_max_idle_conns"`
	ConnectMaxOpenConns int           `mapstructure:"connect_max_open_conns"`
}
...
func LoadConfig(path string) Config {
	...
	if err := viper.ReadInConfig(); err != nil {
		panic(fmt.Errorf("error reading app config: %w", err))
	}

	// Load auth config
	viper.SetConfigName("auth")
	if err := viper.MergeInConfig(); err != nil {
		panic(fmt.Errorf("error reading auth config: %w", err))
	}
